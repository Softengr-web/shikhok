...
app.get('/api/admin/gigs/pending',auth(['ADMIN','SUPER_ADMIN']),handler((req,res)=>ok(res,store.read().gigs.filter(g=>g.moderationStatus==='PENDING'))));
app.post('/api/admin/gigs/:id/moderation',auth(['ADMIN','SUPER_ADMIN']),handler((req,res)=>ok(res,store.transaction(s=>moderateGig(s,actor(req),String(req.params.id),req.body.status,cleanText(req.body.note||'মডারেশন সম্পন্ন','মডারেশন নোট',500))))));
